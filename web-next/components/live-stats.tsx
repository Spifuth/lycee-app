...
export function LiveStats() {
  ...
  const cells: { label: string; value: number | string; accent?: boolean }[] = [
    ...
    { label: 'vote', value: stats.vote_open ? 'ouvert' : 'fermé', accent: stats.vote_open },
  ]

  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <p className="mb-2 font-mono text-xs text-muted-foreground"># live · refresh 30s</p>
          ...
          <div key={c.label} className="text-center">
            ...
            <dd
              className={`mt-1 font-mono text-2xl font-bold ${
                c.accent ? 'text-primary' : 'text-accent'
              }`}
            >
              {c.value}
            </dd>
